...
public struct InterviewReportClient: Sendable {
    /// GET /interview/sessions/{id}/report — 보고서 미생성이면 `InterviewReportError.reportNotFound`.
    public var report: @Sendable (_ sessionId: Int) async throws -> InterviewReport

    public init(report: @escaping @Sendable (_ sessionId: Int) async throws -> InterviewReport) {
        self.report = report
    }
}
...
extension InterviewReportClient: TestDependencyKey {
    public static var testValue: InterviewReportClient {
        InterviewReportClient(report: unimplemented("InterviewReportClient.report"))
    }
