...
// 네트워크 통신 화면의 표준형(템플릿) — Feature 는 Domain Client(JobClient)만 안다.
// NetworkClient/URLSession 은 여기서 보이지 않는 게 정상이다 (Domain Implementation 뒤로 숨는다).
// 실 D14 엔드포인트(GET /api/v1/jobs — [[api#Job]])를 시연한다.
@Reducer
public struct NetworkExampleFeature {
    ...
    public enum Action: ViewAction {
        ...
        /// @CasePathable — 테스트가 `receive(\.inner.jobsLoaded.success)` 로 정밀 매칭하기 위함.
        @CasePathable
        public enum Inner: Sendable {
            case jobsLoaded(Result<[Job], any Error>)
        }
    ...
    public var body: some ReducerOf<Self> {
        Reduce { state, action in
            ...
            case .view(.onAppear), .view(.userTappedRetry):
                ...
                return .run { send in
                    await send(.inner(.jobsLoaded(
                        Result { try await jobClient.jobs() }
                    )))
                }
                .cancellable(id: CancelID.fetch, cancelInFlight: true)

            case let .inner(.jobsLoaded(.success(jobs))):
                state.isLoading = false
                state.jobs = jobs
                return .none

            case .inner(.jobsLoaded(.failure)):
                // Feature 는 CoreNetwork(NetworkError)를 모른다 — 서버 에러코드 구분이 필요해지면
                // Domain 이 도메인 에러로 매핑해 계약에 실어준다.
                state.isLoading = false
                state.errorMessage = "직무 목록을 불러오지 못했어요."
                return .none
