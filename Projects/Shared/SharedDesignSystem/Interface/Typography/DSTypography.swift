...
/// HILIT 타이포그래피 토큰 — Figma «[0722 H/O] HILIT_Text_Guide»(node 988:3149) 스케일과 1:1.
...
public enum DSTypography: String, CaseIterable, Sendable {
    ...
    case body1, body2, body3, body4, body5, body6, body7, body8, body9, body10
}
...
public extension DSTypography {
    var size: CGFloat {
        switch self {
        case .head1, .head2: 32
        case .head3, .head4, .head5, .head6: 24
        case .sub1, .sub2, .sub3: 22
        case .sub4, .sub5, .sub6: 20
        case .sub7, .sub8, .sub9: 18
        case .body1, .body2, .body3, .body4: 16
        case .body5, .body6, .body7: 14
        case .body8, .body9, .body10: 12
        }
    }

    var weight: Pretendard.Weight {
        switch self {
        case .head3, .body1: .bold
        case .head1, .head4, .sub1, .sub4, .sub7, .body2, .body5, .body8: .semiBold
        case .head2, .head5, .sub2, .sub5, .sub8, .body3, .body6, .body9: .medium
        case .head6, .sub3, .sub6, .sub9, .body4, .body7, .body10: .regular
        }
    }

    /// Figma Line Height(%) — head1·head2 120%, head3·head4 135%, 나머지 전부 130%.
    var lineHeightRatio: CGFloat {
        switch self {
        case .head1, .head2: 1.2
        case .head3, .head4: 1.35
        default: 1.3
        }
    }
