...
## List Detail
List → Detail 도메인 내부 네비게이션. `UserListFeature` 행 탭(`delegate.userTappedRow`) → `path.append(.detail(...))`.

## Profile Edit Handoff
⚠️ cross-feature. 상세 화면의 "편집" 버튼은 Profile 을 직접 열지 않는다 — delegate 로 위로 올린다.
...
## 주의사항
import 없는 cross-feature 협업을 유지하기 위한 규칙.
