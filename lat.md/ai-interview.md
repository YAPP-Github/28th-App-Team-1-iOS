...
> 시스템 전체 그림/결정 근거는 [[architecture]], 도메인 큰 그림은 [[domain.map]], Client 분리는 [[clients]] 참고.
...
```
App  (composition root — *ClientLive link → liveValue 활성화)
└── AppFeature  (코디네이터: 탭 + Setup→Session→Report 라우팅)
    ├── InterviewSetupFeature ──┬ QuestionClientInterface
    │   └ Path: jobYears(S0)    ├ PortfolioClientInterface
    │          jd(S1)           └ Models · DesignSystemKit
    │          portfolio(S2)
    │          projectSelect(S3)
    │          loading(S3.5)
    ├── InterviewSessionFeature ┬ QuestionClientInterface
    │   (턴 상태머신)            ├ SpeechClientInterface     (TTS + STT)
    │                           ├ RecordingClientInterface  (A/V 캡처·보존)
    │                           ├ PermissionClientInterface
    │                           └ Models · DesignSystemKit
    ├── PortfolioFeature ─────── PortfolioClientInterface · Models · DSKit
    └── InterviewReportFeature ─ ScoringClientInterface · Models · DSKit
```

단방향 DAG. `Setup`은 `Session`을, `Session`은 `Report`를 **import하지 않는다** — 기존 [[users]]→[[app]]→[[profile]] 핸드오프와 동일.
...
## 3. Client 설계 (외부 IO — 유일하게 Interface/Live 분리, [[clients]])
...
규칙: 각 Client는 `Project.client(name:)`, `testValue`는 전부 `unimplemented`(빈 클로저 금지).
SpeechClient는 책임이 커지면 `TextToSpeechClient` / `SpeechRecognitionClient`로 분리 가능하게 시그니처를 나눠 둔다.

## 4. 도메인 모델 (Shared/Models)
...
| 카운트다운·상태 인디케이터·토스트 | DSKit 컴포넌트 추가 | 🟡 병행 |

## 8. 빌드 순서 (CLAUDE.md "새 모듈 추가 흐름"에 정렬)

1. **Models + DSKit** — 위 도메인 타입 + 면접 전용 컴포넌트(`CountdownRing`, `SpeakingIndicator`, 상태 토스트)
2. **Clients = Interface 먼저**(Live stub) — Portfolio·Question·Speech·Permission·Recording·Scoring, `testValue` unimplemented
