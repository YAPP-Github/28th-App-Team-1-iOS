...
**두 덩어리 — Reducer 2개 + 폴더 2개** (한 모듈 안). `Sources/Home/` 은 `HomeFeature` + `HomeView`(씬) + `HomeReportSheet`(하단 판) + `HomeReportRow`(행 모델의 응답 매핑·픽스처 — 리듀서 파일 길이 제한으로 갈라 뒀다), `Sources/StartInterview/` 은 `StartInterviewFeature` + `StartInterviewView`, 공용 배경·드래그 규칙은 `Sources/Components/`.
...
「면접 시작」 4화면(처음/동일 정보/진행 중/이용권 소진)이 **cover present 가 아닌 이유**: 시트를 끌어 내리는 동안 뒤에서 드러나야 하고, 손을 놓기 전까진 되돌릴 수 있어야 한다(사용자 요구 2026-08-01 — Airbnb 지도/목록 스냅 참조). 그래서 `StartInterviewFeature.State` 는 `@Presents` 옵셔널이 아니라 **늘 붙어 있고**(`Scope`), 보이는 정도는 시트 높이가 정한다. `StartInterviewView` 도 화면이 아니라 배경·내비바 없는 **한 겹**이다.
...
홈 밖으로 나가는 6건은 delegate — `profileRequested`(마이페이지) · `reportDetailRequested(id:)`(리포트 상세) · `interviewStartRequested`·`interviewInfoEditRequested`·`interviewRestartRequested`·`interviewResumeRequested`(StartInterview 의 `startRequested`·`editInfoRequested`·`restartRequested`·`resumeRequested` 를 올린 것). AppFeature 가 여섯 케이스를 명시로 받고, 시작·수정 두 건은 온보딩 위저드 cover 로 이어진다(`state.onboarding` — [[app]]). 나머지 넷은 아직 `.none` + TODO 다 — 막힌 지점이 경계 한 곳에 모인다. 진행 중 두 건은 **세션 id 를 싣지 않는다**: held 세션 조회 API 가 없어 홈이 그 값을 모른다(미결 6-3). 펼침 토글(`userTappedReportRow`)은 홈 내부 상태라 delegate 가 아니다.
...
로드값 → 표시 매핑은 `HomeFeature` 의 두 헬퍼가 전담한다. `reusablePortfolio` 는 **READY 만** 고르고(PROCESSING 을 걸면 시작 시점에 게이트가 뒤집는다 — 폴링 승격은 TODO), `startVariant` 는 잔여 0 을 최우선(`exhausted`)으로 포폴 유무를 갈라 시안 3종을 정한다 — **잔여 nil(프로필 실패·응답 전)은 0 이 아니다**: 모른다고 소진 시안을 띄우면 시작 경로가 [홈으로] 하나로 막힌다. 목록 응답 → 행 매핑은 `Report.init(summary:)` 하나다(날짜 «7월 11일 토» 는 **KST·ko_KR 고정 포맷** — 서버가 타임존 없는 LocalDateTime 을 주므로 표시만 기기 로컬로 두면 하루 밀린다). 남은 1종(진행 중 held 세션)은 계약 대기라 미배선 — 화면(`inProgress` + 확인 단계)만 2026-08-07 선구현이라 `startVariant` 가 그 변형을 내지 못하고 프리뷰로만 볼 수 있다. phase 는 서버 판정의 표시일 뿐, 진실은 탭 시점 게이트 재검증이다.
