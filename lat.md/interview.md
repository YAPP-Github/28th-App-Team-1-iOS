...
- 좌상단 X 는 8:00 전이면 중도 이탈 경고(«지금 나가면 이용권 1회가 차감돼요» — 리포트 언급 금지), 8:00 후면 종료 확인 모달(Figma 2555:7696)로 갈린다. 둘 다 destination 없이 Bool 오버레이이고 같은 모달 컴포넌트를 쓴다. 8:00 해금 틱이 열려 있던 경고를 닫아 «경고를 띄운 채 해금» 레이스를 막는다.
