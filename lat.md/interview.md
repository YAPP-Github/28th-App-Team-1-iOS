...
- 정지 지점: 준비→실패, 세션→홈(정상 종료), 세션→실패, 세션 중도 이탈. 실패·이탈 전환은 재생(stopPlayback)과 **녹화 폐기**(`discardRecording`)도 함께 하고, 정상 종료 전환만 재생을 살리고(마무리 멘트) 폐기도 부르지 않는다 — 파일 소유권이 [[interview#업로드 큐]] 로 넘어가서다. 동결의 `interrupted` 이탈도 폐기하지 않는다 — 세그먼트가 재개 재료다([[interview#코디네이터]]). 재진입은 Readiness onAppear 가 다시 켠다.
