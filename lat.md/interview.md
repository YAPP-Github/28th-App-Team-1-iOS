...
**소유 분담**(2026-08-08 배선 완료 — #69 «녹화하며 갱신» 이 해소된 지점이다): 앱 코디네이터([[app#Cross-feature Routing]])가 생성 save(0초)·완주 clear·홈 두 갈래(abandon 성공·재개 ENDED)의 clear·킬 클린업 clear 를 맡고, 면접 쪽은 백그라운드 마감의 **갱신**과 오버레이 중단(`NETWORK_DISCONNECT`) 뒤 clear([[interview#세션]])·복귀 ENDED 판정의 clear([[interview#코디네이터]])를 맡는다. `processToken` 은 «이 프로세스가 세그먼트를 들고 있다» 는 표식(프로세스당 상수 `currentProcessToken`)이고, 재개 제안 판정 `isResumableInCurrentProcess`(0초 ‖ 현재 프로세스)가 죽은 프로세스의 진행분을 걸러 앞부분 없는 영상 재개를 막는다 — 세그먼트가 tmp·프로세스 수명이라서다([[interview#프리뷰]]). 옵셔널이라 구버전 저장값은 nil 로 디코딩돼 «죽은 값» 으로 접힌다. 킬 클린업의 판정은 Interface 순수 함수 `HeldSessionCleanup` 이 맡는다(효과는 AppFeature 가 싣는다): `target` 은 죽은 프로세스의 진행분만 고르고(0초 보관분·현재 프로세스 값은 불가침), `followup` 은 RESUMABLE → `abandon(USER_EXIT)` · ENDED → 로컬 정리로 가른다. App 테스트 타깃이 없어 결정표는 `HeldSessionCleanupTests`·`HeldSessionTests` 가 유닛으로 고정한다.
