...
## 디자인 시스템
`Shared/SharedDesignSystem` 토큰 우선, 하드코딩 지양. 타이포그래피는 Figma «[0722 H/O] HILIT_Text_Guide» 스케일과 1:1 — `DSTypography`(head1~6·sub1~9·body1~10, Pretendard 4웨이트) + `.dsTypography(_:)` 모디파이어(행간·자간 포함). 색·spacing·outline 토큰은 2-tier 로 구현(컴포넌트 토큰은 이관 대기).
...
- **스펙 대조**: 토큰 ↔ Figma 스타일명 1:1 은 `DSTypographyTests` 가 고정한다. 상세 토큰 표 → `.claude/design/typography.md` (인덱스: `.claude/design.md`). 시맨틱 별칭은 `DSColorTests` 가 고정, 토큰 값 단일 소스는 코드(enum)·Figma 대조. 상세 → `.claude/design/color.md`·`.claude/design/spacing.md`.
