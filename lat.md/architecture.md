...
## 디자인 시스템
`Shared/SharedDesignSystem` 토큰 우선, 하드코딩 지양. 타이포그래피는 `DSTypography`(25종, Pretendard 4웨이트) + `.dsTypography(_:)` — Figma «[0722 H/O] HILIT_Text_Guide» 1:1. 색·spacing·outline 토큰 구현 완료, 컴포넌트는 버튼 체계(`ButtonLarge` + ButtonStyle 4종)와 그 밖 6종.
