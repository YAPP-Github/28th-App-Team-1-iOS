...
1. `OnboardingFeature` 가 분석까지 끝내고 `delegate(.finished(sessionId))` 방출 → 위저드를 닫고 `state.interview = InterviewFeature.State(sessionId:)` 로 fullScreenCover 제시 (`.dismiss` 는 세션이 없어 닫기만)
2. 면접 종료 두 신호 모두 `state.interview = nil` — 커버 아래가 이미 홈이라 별도 라우팅이 없다. 목적지가 같아도 케이스를 합치지 않는다: `.finished`(리포트 대기 «홈으로»)는 홈 리포트 목록 갱신이 붙을 자리고 `.closed`(8:00 전 이탈·실패 화면 X)는 아니다 → [[interview#코디네이터]]
