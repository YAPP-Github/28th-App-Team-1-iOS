...
3. 온보딩 `delegate(.finished(sessionId:))` = **세션 준비 완료** → 위저드 cover 를 닫고 그 자리에서 `state.interview = InterviewFeature.State(sessionId:)` 로 면접 cover 를 열고, **`HeldSessionStore.save(sessionId, 0초)`** 로 진행 중 보관을 시작한다(이 값의 존재가 홈 «진행 중» 판정 재료 — [[interview#Client 계약]]. 이후 갱신은 면접 Feature 가 백그라운드 마감마다 누적초+프로세스 토큰으로 한다, 2026-08-08 배선 — [[interview#세션]]) → [[interview]]
