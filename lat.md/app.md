...
3. 온보딩 `delegate(.finished(sessionId:))` = **세션 준비 완료** → 위저드 cover 를 닫고 그 자리에서 `state.interview = InterviewFeature.State(sessionId:)` 로 면접 cover 를 연다. **진행 중 보관은 여기서 시작하지 않는다**(2026-08-18 이동) — 준비 화면은 아직 면접 전이라 여기서 심으면 시작하기를 누르지 않고 나간 사용자에게도 홈이 «진행 중» 카드를 그렸다. `HeldSessionStore.save(sessionId, 0초·표식 없음)` 는 면접 Feature 의 시작하기 탭이 하고, 표식은 세션 화면이 녹화를 열 때, 누적초는 백그라운드 마감마다 찍는다([[interview#코디네이터]]·[[interview#세션]]) → [[interview]]
