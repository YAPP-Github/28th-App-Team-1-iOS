...
## Users ↔ Profile
가장 중요한 cross-feature 흐름이자 import 그래프엔 안 보이는 의존. **둘은 서로 import 하지 않는다** — 전부 delegate + AppFeature 중재.
...
## Feature ↔ Client
각 Feature 가 의존하는 Client(Interface) 매핑.
...
## 계획 — AI 면접
YAPP APP 1팀 「AI 면접 연습 앱」을 우리 아키텍처에 녹인 후속 도메인 설계(현재 데모 4탭과 별개) — Setup/Session/Report Feature + Client 군.

- 전체 개요·Part 1/2 → [ai-interview](../docs/work/ai-interview.md)
- **Part 3 분석 보고서 & 영상 복기** → [ai-interview-report](../docs/work/ai-interview-report.md). `InterviewReportFeature`(R0·R1 + V0·V1·V2, 자체 Path). 신규 Client: PlaybackClient(영상 자산·재생 시간축) · ReviewClient(자기평가 영구 저장 → 4.6 '나'), ScoringClient 확장(폴링·기준선).
