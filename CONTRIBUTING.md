# 팀 컨벤션 (2인 협업 · iOS / Tuist TMA)

> 「개발 요청 문서」 §1 의 결정 기록. 작업 세부 규칙은 `CLAUDE.md`, 제품/도메인 지식은 `lat.md/` 참고.

## 1.1 브랜치
- **전략**: GitHub Flow — `master` + 짧은 수명 `feature/*`.
- **보호**: `master` 직접 push 금지 · force push 금지 · PR 필수.
- **수명**: 며칠 내 머지. 길어지면 잘게 쪼갠다.

## 1.2 네이밍 / 커밋 / PR
- **브랜치명**: `{type}/#{이슈번호}_{짧은-설명}` — `feature/` `fix/` `refactor/` `experiment/` `chore/`. 예: `feature/#7_릴스댓글시트`.
- **커밋**: `type: 설명_부연` — **제목 1줄 한국어**. type ∈ feat/fix/refactor/rename/docs/test/chore. (기존 규칙 유지, Conventional Commits 미채택) 두 type 에 걸치면 커밋을 쪼갠다 — `chore`(빌드/툴/메타만) vs `refactor`(코드 의미 변경) vs `rename`(심볼·파일명만 변경, 동작 그대로) 구분. 본문 bullet 은 "무엇이 바뀌었는가"를 적고 필요할 때만.
...
## 1.4 배포 / 권한 / 시크릿 / 롤백
- **환경**: `Dev`/`QA`/`Release` 빌드 Configuration (계별 xcconfig·번들 분리 — DocC `Environments` 아티클) · TestFlight(QA) · App Store(Release).
