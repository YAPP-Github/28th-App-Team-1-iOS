...
| S1 JD | 3 JDLink — 링크/직접입력 탭 (상호배타) | 선택 — 스킵 상시 | ✅ (검증 일부 TODO) |
| S2 포트폴리오 | 4 PortfolioUpload — 202+폴링 | 필수 | ✅ (선검증·상한 TODO) |
| S3 집중 프로젝트 | 5 FocusProject — 자유입력 | 선택 — 스킵 상시 | ✅ |
...
- **3 JD** — 링크 검증(디바운스 → `validate`) ✅ · 성공 시 직접입력 탭 잠금 ✅. TODO: ① 직접입력 **200~3,000자** 검증(다음 활성 조건 — 200 미만/3,000 초과 문구 확정본 있음) ② 링크 본문 <200자 = `CONTENT_TOO_SHORT` 문구 노출 ③ **링크 검증 1일 5회 제한** 초과 에러 노출(서버 에러 코드 확인) ④ **스킵 시 입력이 있어도 검증·저장 없이 통과**(jd=nil — 현재 동작 확인).
- **4 포트폴리오** — 클라 선검증은 UX 용 빠른 차단, **최종 판정은 서버 실측**(PRD §7 분담): PDF 타입·20MB ✅ / **페이지 ≤30**(PDFKit `pageCount`) TODO / **암호 PDF**(`PDFDocument.isEncrypted` — "가능하다면" 항목) TODO. 글자 수 ≥30 은 서버 전용(Tika) → FAILED_FILE 문구만. 폴링 3초 ✅, **폴링 상한**(전체 처리 타임아웃 → FAILED_SYSTEM 취급 문구, 초기값 tentative) TODO. **1개 제한**: `PORTFOLIO_ALREADY_EXISTS` → "기존 삭제 후 재업로드" dialog(자동 교체 금지) TODO. 셀룰러에서 20MB 업로드 경고는 "가능" 항목 — 후속.
- **5 집중 프로젝트** — 10~300자(하한 10자를 클라 활성 조건에도 반영할지 확인 — 현재 서버 위임) · 빈 입력 = 스킵(nil) ✅ · 상단 고정 문구는 교체 확정본("입력하면 그 부분을 집중 검증해요…") 반영.
