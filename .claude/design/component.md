...
| `.medium(_:)` | ButtonMedium 1941:3261 | `.buttonStyle(.medium(.green))` | h45·px24/py12·hug. 색 6종 default/black/gray/green/blue/red |
...
| `.hilitButtonLoading(_:)` | — | `ButtonLarge(…).hilitButtonLoading(true)` | 라벨을 감추고 스피너를 얹어 폭 유지 + 탭 차단 |

**설탕 래퍼 3종** — 본체는 위 체계이고, 텍스트 한 줄짜리 흔한 경우를 짧게 쓰라고 남긴 것. 새 조합이 필요하면 본체를 직접 쓴다.

| 래퍼 | 위임 |
|---|---|
| `PrimaryButton(_ title:, isLoading:, action:)` | `ButtonLarge(title, .bottom)` + `.hilitButtonLoading` |
| `ModalButton(_ title:, action:)` | `ButtonLarge(title, .modal)` |
| `MiniButton(_ title:, icon: Image?, action:)` | `.buttonStyle(.mini(.gray, layout:))` — 아이콘 유무로 layout 자동 |
