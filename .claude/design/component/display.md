...
| `.hilitBottomSheet` | 477:6341 (전문 시트 — 폭 꽉·모서리 0·머리 그래버) · 딤은 modal 2302:6080 과 공유 | 모디파이어 — `(isPresented:, detents:, surface: .white, showsGrabber: true, onDismiss:) { 본문 }` / `(item:, …) { sheet in switch … }` | 바텀시트 표출 — **시스템 `.sheet` 를 쓰지 않는다**(iOS 26 이 부분 높이 시트를 가장자리에서 띄워 그려 시안의 «폭 꽉·모서리 0» 판이 다른 물건이 됐다 — 디자인 반려, 이슈 #72). **DS 가 시트 크롬을 갖는다** — 딤(`HilitDim`, `.hilitModal` 과 공유)·자리·드래그·그래버·폭·모서리 0 이 여기 몫이고 호출부는 판 색(`surface`)과 본문만 준다(예전의 «껍데기만» 계약은 시트 룩이 화면마다 갈라져 접었다). `detents` 는 화면 높이 대비 비율의 **오름차순** 배열(`[662.0/812.0, 1]`) — 가장 작은 자리로 열리고 그래버를 끌어 나머지를 오간다. 원소 하나면 고정 높이. 착지·닫기 판정은 `HilitBottomSheetMotion`(임계 60pt + 관성 10%), 판은 아래로 끌면 **높이 유지한 채 offset**(줄이면 본문이 다시 흘러 덜컹거린다) 위로 끌면 자란다. 표출 층·제약은 `.hilitModal` 과 같다(아래 «표출 층» — cover, 화면당 하나). **값 기반** — 스스로 닫지 않고 사용자가 내린 것(딤 탭·아래로 끌기)만 `onDismiss` 로 되돌린다. 시트 2개↑ 화면은 `item:` 에 enum 하나. 사용처 AuthTerms 전문 시트 |
