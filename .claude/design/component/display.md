...
| `SheetGrabber` | 바텀시트 머리 막대 — 477:6341 · «Home_Report_Sheet» | `()` | 바텀시트 손잡이 — g400 60×5 막대(모서리 0)를 높이 20 줄 가운데. 시스템 `presentationDragIndicator`(캡슐)와 규격이 달라 직접 그린다. **제스처 없음** — 끌어서 자리를 옮기는 판정은 시트 높이를 소유한 쪽에 있다. `.hilitBottomSheet` 는 이 줄을 스스로 얹고 제스처도 자기가 붙이므로 호출부는 손댈 게 없고, 직접 쓰는 건 화면에 상주하는 시트(홈 리포트 판 — 모디파이어를 안 쓴다)뿐이다. 줄 전체(h20)가 `contentShape` 라 막대 밖 빈자리까지 잡힌다 |
