...
| `InfoField` | info-field 2085:3925 | `(_ text:, style: .gray/.error)` | 입력·화면 아래 안내/에러 줄 — 아이콘 16 + 12pt, px14/py12·모서리 0. `.gray` g100 판 + 검정 «원 안 i» / `.error` e200 판 + e300 테두리 1.2 + **채운 빨간 원 안 흰 느낌표**(`issue/16px/error`). 아이콘은 판 색에 묶여 있다(시안 instance-swap 슬롯을 열지 않음), 폭은 호출부 몫. `.error` 글리프는 마스터(2085:3924, 원 안 i)와 실사용 인스턴스가 갈리는데 **다수인 인스턴스 쪽**을 따랐다 — 아래 «Figma 원본 불일치» ⑤ |
...
① `Checkbox` 의 꺼짐 변형 이름이 `status=Indeterminate`(세 번째 상태가 아니라 그냥 off) — off 로 구현했다. ② 같은 변형의 체크 위치가 켜짐보다 x·y 각 1.6 (= 테두리 두께) 위/왼쪽이다 — 테두리 안쪽 기준으로 놓인 실수로 보고 두 상태 모두 켜짐 위치(가로 정중앙)로 맞췄다. ③ `text-field` 의 loading 라벨(«분석 중») 색이 컴포넌트 세트(2044:1798)는 g400 인데 카운터 조합 예시(2286:5661)는 g900 — 세트 쪽(g400)으로 구현했다. ④ `text-field` 두 패밀리의 패딩이 상충한다 — `435:1603`(small/large)은 네 변 p14, `435:1630`(case 패밀리)은 px16/py14. 코드는 case 패밀리를 따른다(현행 유지). ⑤ `info-field` 의 **빨간 판 아이콘이 마스터와 인스턴스에서 다르다** — 마스터(2085:3924)는 «원 안 i» 를 e500 으로 덮어쓴 것이고(그래서 아이콘 시트에 이름 붙은 error 변형이 없다), 실제로 배치된 빨간 안내줄은 전부 «issue/16px/error»(채운 원 + 흰 느낌표)를 끼워 넣는다: Part5 업로드 불가 모달 435:8895 · 업로드 실패 카드 아래 줄 439:13132·439:13299. 3:1 로 다수인 인스턴스 쪽으로 구현했다(2026-08-01, 사용자 확인).
