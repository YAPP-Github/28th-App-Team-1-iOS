...
`Image` 토큰 레퍼런스. 단일 소스는 [Image+Tokens.swift](../../Projects/Shared/SharedDesignSystem/Interface/Image/Image+Tokens.swift) — 어긋나면 코드가 우선이고 이 문서를 갱신한다. Color 팔레트와 같은 패밀리 enum 접근.
...
- **에셋 로드**: 값은 `Interface/Resources/Assets.xcassets`(에셋명 = `Ic*`/`Img*` 프리픽스), 로드는 Tuist 생성 접근자 `SharedDesignSystemInterfaceAsset.Assets.<name>.swiftUIImage` ([Image+Tokens.swift](../../Projects/Shared/SharedDesignSystem/Interface/Image/Image+Tokens.swift) 가 `Asset` 으로 축약해 감싼다). 생성 이름은 크기·틴트 여부를 담지 못하므로 그 정보는 토큰 주석이 진다. 에셋을 지우거나 이름을 바꾸면 토큰에서 컴파일 에러가 난다.
