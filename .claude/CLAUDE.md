...
Tuist/ProjectDescriptionHelpers/         ← Project.feature/.domain/.core + 타입드 의존 액세서
Projects/
├── App/                                   composition root
│   ├── Sources/ArchitectureApp.swift        @main — *DomainLive link → liveValue 활성화
│   ├── AppFeature/                          탭 코디네이터 + cross-feature 라우팅
│   └── Documentation/                       ArchitectureDocs 타겟 — 전역 DocC 카탈로그 (코드 없음)
├── Feature/{Home,Users,Profile,Activity}/   화면 도메인 (Sources+Testing+Tests+Example+Interface)
├── Domain/{User,Profile,Activity}/          도메인 모델 + Repository (Interface + Live 분리)
└── Shared/{DesignSystemKit}/                디자인 토큰
```

의존 방향: `App → AppFeature → Feature* → Domain*Interface`. 전 모듈이 `DesignSystemKit` 의존 가능.
...
- **Feature 는 `Domain*Interface` 만 의존.** `Domain*Live`(실제 구현)는 **App 타겟 / Example 앱만** link. Feature·AppFeature 는 Live 를 절대 import 하지 않는다.
- **Domain 만 Interface/Live 분리.** Feature 는 단일 모듈(+Testing/Tests/Example), Interface/구현 분리 안 함.
...
1. (모델 + Repository) `Projects/Domain/Xxx/{Interface/Sources/,Live/Sources/}` + `Project.swift` → `Project.domain(name: "Xxx", liveDependencies: […])`
2. (화면) `Projects/Feature/Xxx/{Sources,Testing,Tests,Example}/` + `Project.swift` → `Project.feature(name: "Xxx", dependencies: […], exampleDependencies: […])`
...
- 테스트: `make test scheme=FeatureHome` 권장 (사용 가능한 시뮬레이터 UDID 로 해석 — 기기 이름이 여러 OS 런타임에 중복돼도 안전). 직접 xcodebuild 쓸 때 `name=iPhone 16` 이 중복으로 "Unable to find a device" 나면 `,OS=18.0` 또는 `id=<UDID>` 로 지정
- Feature 단독 실행: `Feature*Example` 스킴 (예: `FeatureActivityExample`)
